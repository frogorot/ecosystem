...
Animal Bug1(int x, int y, int d){
	...
	return Animal(0, 3.0, 1.0, 9.5, color, Vector(x, y), dir); 
		...
		//Animal(1.0,8.0,4.0,60.0, color, Vector(x, y), dir); //37
		//Animal(1.0,3.0,1.0,9.0, color, Vector(x, y), dir); //5.5 при каком-то S порождает выживание первого жука, хот€ D/2<V+2L
		//Animal(1.0,3.0,1.0,11.0, color, Vector(x, y), dir); //12 // забавный старт, потом оп€ть квадраты
